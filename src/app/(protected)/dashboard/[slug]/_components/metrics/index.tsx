...
const Chart = (props: Props) => {
  ...
  const stats = useMemo(() => {
    if (!data?.data) return { total: 0, active: 0, interactions: 0 };
    
    const totalAutomations = data.data.length;
    const activeAutomations = data.data.filter((automation: any) => automation.listener).length;
    const totalInteractions = data.data.reduce((sum: number, automation: any) => {
      ...
      <div className="relative overflow-hidden rounded-xl bg-muted border border-border p-6">
        ...
        {stats.total > 0 ? (
          ...
          <div className="relative z-10 text-center py-8">
            ...
            <a href={`/dashboard/${slug}/automations`} className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-brand text-white text-sm font-medium hover:shadow-glow transition-all">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">

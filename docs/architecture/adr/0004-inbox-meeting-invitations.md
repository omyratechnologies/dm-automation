...
One serializable PostgreSQL transaction creates the hash-protected, fragment-secret Booking Link, queued outbound Message, audit record and identifier-only `MessageQueued` outbox event. After commit, the relay loads the message and tenant-scoped routing identifiers from PostgreSQL and enqueues `send-messages:<eventId>`. The existing sender remains the single Meta policy and rate-limit enforcement point.

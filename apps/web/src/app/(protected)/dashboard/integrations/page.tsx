      ...
      {bindings.map((binding) => {
        ...
        return <div key={binding.id} className="rounded-lg border border-border p-3">
          <div className="flex flex-col gap-3">
            <div className="min-w-0"><p className="flex items-start gap-2 text-sm font-medium">{binding.status === "ACTIVE" ? <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" /> : <TriangleAlert className="mt-0.5 h-4 w-4 shrink-0 text-amber-600" />}<span className="break-all">{email}</span></p><p className="mt-1 text-xs text-muted-foreground">{binding.ownership.toLowerCase()} authorization · {binding.status.replaceAll("_", " ").toLowerCase()}</p>{binding.lastErrorCode && <p className="mt-1 text-xs text-amber-700 dark:text-amber-300">Needs attention: {binding.lastErrorCode.replaceAll("_", " ").toLowerCase()}</p>}</div>

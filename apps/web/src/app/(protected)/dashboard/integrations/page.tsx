...
export default function IntegrationsPage() {
  ...
  const destinations = useQuery<Destination[]>({ queryKey: ["sheet-destinations", workspaceId], queryFn: () => api(wsPath("/sheets/destinations")), enabled: !!workspaceId && readiness.data?.sheets.available === true });
  ...
  return <div className="space-y-5 pb-10"><PageHeader title="Integrations" description="One secure Google connection center with independent Calendar and Sheets controls." icon={<ShieldCheck className="h-5 w-5" />} />
    {bindings.isLoading ? <div className="grid gap-4 lg:grid-cols-2"><Skeleton className="h-72 rounded-xl" /><Skeleton className="h-72 rounded-xl" /></div> : <div className="grid gap-4 lg:grid-cols-2">
      <IntegrationCard icon={<CalendarDays className="h-5 w-5" />} title="Google Calendar" description="Availability, owner-first host routing, Meet links and conflict reconciliation." bindings={calendarBindings} canManage={canManage} availability={readiness.data?.calendar} readinessFailed={readiness.isError} isConnecting={connect.isPending} onConnect={() => connect.mutate(["CALENDAR"])} consequences="Disconnecting stops new routing immediately. Existing Google invitations remain with attendees." />
      <IntegrationCard icon={<FileSpreadsheet className="h-5 w-5" />} title="Google Sheets" description="Controlled two-way lead views with version checks and inspectable conflicts." bindings={sheetBindings} canManage={canManage} availability={readiness.data?.sheets} readinessFailed={readiness.isError} isConnecting={connect.isPending} onConnect={() => connect.mutate(["SHEETS"])} consequences="Disconnecting pauses projections. Sheet rows and Google version history are not automatically erased." />
    </div>}
...
function IntegrationCard({ icon, title, description, bindings, canManage, availability, readinessFailed, isConnecting, onConnect, consequences }: { icon: React.ReactNode; title: string; description: string; bindings: Binding[]; canManage: boolean; availability?: GoogleAvailability; readinessFailed: boolean; isConnecting: boolean; onConnect: () => void; consequences: string }) {
  ...
  return <Card><CardHeader><div className="flex items-center justify-between gap-3"><CardTitle className="flex items-center gap-2 text-base">{icon}{title}</CardTitle><Badge variant={healthy ? "default" : "secondary"}>{badge}</Badge></div><CardDescription>{description}</CardDescription></CardHeader><CardContent className="space-y-4">{bindings.map((binding) => <div key={binding.id} className="rounded-lg border border-border p-3"><p className="flex items-center gap-2 text-sm font-medium">{binding.status === "ACTIVE" ? <CheckCircle2 className="h-4 w-4 text-emerald-600" /> : <TriangleAlert className="h-4 w-4 text-amber-600" />}{binding.grant.email ?? "Google account"}</p><p className="mt-1 text-xs text-muted-foreground">{binding.ownership.toLowerCase()} authorization · {binding.status.replace("_", " ").toLowerCase()}</p></div>)}<p className="text-xs leading-5 text-muted-foreground">{consequences}</p>{unavailableCopy && <p className="rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs leading-5 text-amber-700 dark:text-amber-300" role="status">{unavailableCopy}</p>}{canManage && <Button className="h-11 w-full" disabled={!availability?.available || readinessFailed || isConnecting} onClick={onConnect}>{isConnecting ? "Opening Google…" : "Connect or expand access"}</Button>}{!canManage && <p className="text-xs text-muted-foreground">An Owner or Admin manages this connection.</p>}</CardContent></Card>;
}

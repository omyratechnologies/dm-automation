...
export const useEditAutomation = (automationId: string) => {
  ...
  // Use useCallback to memoize disableEdit
  const disableEdit = React.useCallback(() => setEdit(false), []);
...
export const useAutomationPosts = (id: string) => {
  ...
  const onSelectPost = (post: {
    ...
    setPosts((prevItems) => {
      if (prevItems.find((p) => p.postid === post.postid)) {
        ...
        return [...prevItems, { ...post, requireFollow: false }];
      }
    });
  };

  const onToggleRequireFollow = (postid: string) => {
    setPosts((prevItems) =>
      prevItems.map((p) =>
        p.postid === postid ? { ...p, requireFollow: !p.requireFollow } : p,
      ),
    );
  };
  ...
  return { posts, onSelectPost, onToggleRequireFollow, mutate, isPending };
};

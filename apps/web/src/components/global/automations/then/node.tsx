...
const ThenNode = ({ id }: Props) => {
  ...
  const commentTrigger = (data?.trigger ?? []).find((t: { type: string; }) => t.type === "COMMENT");

  return !data?.listener ? (
    <></>
  ) : (
    <div className="w-full lg:w-10/12 relative xl:w-6/12 p-5 rounded-xl flex flex-col bg-card dark:bg-[#1D1D1D] border border-border gap-y-3">

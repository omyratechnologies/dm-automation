...
export const sendDM = async (
  ...
  logger.info("Sending DM", { userId, recieverId });
  ...
  if (!rateLimitCheck.success) {
      logger.error("Instagram API rate limit exceeded", { reset: rateLimitCheck.reset });
      throw new Error(
...
export const sendPrivateMessage = async (
  ...
  logger.info("Sending private message to comment", { userId, commentId: recieverId });
...
export const generateTokens = async (code: string) => {
  ...
  logger.info("Token exchange response received", {
    hasToken: !!tokenData.access_token || !!tokenData.data?.[0]?.access_token,
  ...
  if (tokenData.error_type || tokenData.error) {
    logger.error("Token exchange failed", {
      error: tokenData.error_message || tokenData.error?.message,
    });
  ...
  logger.info("Got short-lived token", { userId: shortToken.user_id });
  ...
  try {
    ...
    logger.info("Long-lived token acquired", { expiresIn: longTokenRes.data.expires_in });
